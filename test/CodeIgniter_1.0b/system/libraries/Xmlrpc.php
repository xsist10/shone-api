...
|----------------------------------------------------------
| File: libraries/xmlrpc.php
|----------------------------------------------------------
| Purpose: XML-RPC request handler class
|==========================================================
*/
...
class _XML_RPC {
	...
	function _XML_RPC ($config = array()) 
	{
...
class XML_RPC_Client extends _XML_RPC
{
	var $path			= '';
	var $server			= '';
	var $port			= 80;
	...
	function decode($array=FALSE)
	{
		if ($array !== FALSE && is_array($array))
		{
			while (list($key) = each($array))
			{
				...
				{
					$array[$key] = $this->input->xss_clean($array[$key]);
				}
			}
		...
		else
		{
			...
			{
				$result = $this->input->xss_clean($result);
			}
		}
...
class XML_RPC_Message extends _XML_RPC
{
	var $payload;
	var $method_name;
	var $params			= array();
	...
	function output_parameters($array=FALSE)
	{
		...
		else
		{
			...
			for ($i = 0; $i < sizeof($this->params); $i++)
    		{
    			...
    			{
    				$parameters[] = $this->input->xss_clean($a_param);
    			}
    		}	
    	}
...
class XML_RPC_Values extends _XML_RPC
{
	var $me 	= array();
	var $mytype	= 0;
